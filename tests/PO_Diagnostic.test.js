    ...
    try {
        ...
        const addressQuery = `
            SELECT DISTINCT
                ...
                -- Valores que se usarían en la query final (con NULLIF para manejar cadenas vacías)
                ISNULL(NULLIF(RTRIM(F.CITY),''),'${DEFAULT_ADDRESS_CITY}') as FINAL_CITY,
                ISNULL(NULLIF(RTRIM(F.COUNTRY),''),'${DEFAULT_ADDRESS_COUNTRY}') as FINAL_COUNTRY,
                ISNULL(NULLIF(RTRIM(F.[LOCATION]),''),'${DEFAULT_ADDRESS_IDENTIFIER}') as FINAL_IDENTIFIER,
                ISNULL(NULLIF(RTRIM(F.ADDRESS2),''),'${DEFAULT_ADDRESS_MUNICIPALITY}') as FINAL_MUNICIPALITY,
                ISNULL(NULLIF(RTRIM(F.[STATE]),''),'${DEFAULT_ADDRESS_STATE}') as FINAL_STATE,
                ISNULL(NULLIF(RTRIM(F.ADDRESS1),''),'${DEFAULT_ADDRESS_STREET}') as FINAL_STREET,
                ISNULL(NULLIF(RTRIM(F.ZIP),''),'${DEFAULT_ADDRESS_ZIP}') as FINAL_ZIP,
                CASE 
