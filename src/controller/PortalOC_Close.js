...
async function cancellationPurchaseOrders(index) {
    ...
    `;
        ...
        try {
            ...
            console.log(
                `📤 [${i + 1}/${recordset.length}] PO ${ponumber} cerrada en portal\n` +
                `   ▶ Endpoint: ${endpoint}\n` +
                `   ▶ Status:   ${resp.status} ${resp.statusText}`
            );
        } catch (err) {
            console.error(`🚨 [${i + 1}/${recordset.length}] Error cerrando PO ${ponumber}:`);
            if (err.response) {
    ...
    `;
        try {
            ...
            console.log(`✅ PO ${ponumber} marcada CLOSED en FESA`);
        } catch (err) {
