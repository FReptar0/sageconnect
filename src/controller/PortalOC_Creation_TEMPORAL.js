...
// Órdenes específicas a procesar (desde 2024)
const SPECIFIC_POS = ['PO0074902', 'PO0075300'];

async function createSpecificPurchaseOrders(index) {
  const today = getCurrentDateString(); // 'YYYY-MM-DD'
  const logFileName = 'PortalOC_Creation_TEMPORAL';
  
  console.log(`[INICIO] Ejecutando proceso temporal para órdenes específicas - Tenant: ${tenantIds[index]} - Fecha: ${today}`);
  console.log(`[INFO] Procesando POs: ${SPECIFIC_POS.join(', ')}`);
  
  // 1) Ejecuta tu consulta a DATABASE para los POs específicos (sin filtro de fecha)
  const poFilter = SPECIFIC_POS.map(po => `'${po}'`).join(',');
...
`;
  ...
  try {
    ...
    logGenerator(logFileName, 'info', `[INFO] Iniciando createSpecificPurchaseOrders para index=${index}. Total de registros recuperados: ${recordset.length}`);
  } catch (dbErr) {
  ...
  console.log(`[FIN] Proceso temporal completado para tenant ${tenantIds[index]}`);
}

// Ejecutar para el primer tenant (index 0)
createSpecificPurchaseOrders(0).catch(err => {
  console.error('❌ Error en createSpecificPurchaseOrders:', err);
});

module.exports = {
  createSpecificPurchaseOrders
}