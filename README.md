...
| DEFAULT_ADDRESS_IDENTIFIER | Default location identifier for purchase orders when ICLOC table has no data. Leave empty if no default needed. | SERVIC |
| DEFAULT_ADDRESS_MUNICIPALITY | Default municipality for purchase orders when ICLOC table has no data. Leave empty if no default needed. | EJIDO HACIENDA NUEVA |
| DEFAULT_ADDRESS_STATE | Default state for purchase orders when ICLOC table has no data. Leave empty if no default needed. | ZACATECAS |
| DEFAULT_ADDRESS_STREET | Default street for purchase orders when ICLOC table has no data. Leave empty if no default needed. | CERRO DE SAN ROBERTO S/N, |
| DEFAULT_ADDRESS_ZIP | Default ZIP code for purchase orders when ICLOC table has no data. Leave empty if no default needed. | 98100 |
